...
| 이름 | 역할 | 실루엣 | 원본 팩 |
|---|---|---|---|
| **SHINOBI** | 주인공. 검이 기본, 수리검이 소모품, 대쉬로 거리를 번다 | 청색 갑주, 가장 좁은 몸 | Ninja Adventure |
| **IRONCLAD** | 중장. 접근 후 윈드업–스윙, 중거리에서 런지 | 투구·깃털·방패, 최대 어깨폭 | The Knight |
| **SHADE** | 경장 돌격. 달려와 도약하고 벽을 넘어온다 | 보라, 가장 얇음 | Ninja Girl |
| **GLAIVE** | 고정 원거리. 조준 후 강철 초승달을 던진다 | 녹색, 낮은 무게중심 | Ninja Adventure |
...
그 위에 **CC0 무료 애셋으로 만든 아틀라스**가 올라갑니다 (`assets/sprites.png`, 49KB).
`frontend/tools/pack_assets.py`가 CC0 팩 3종을 받아 게임 자체 팔레트로 리컬러해서 굽습니다.
라이선스·출처·거부한 후보는 `frontend/assets/README.md`의 장부에 전부 기록돼 있습니다.
...
frontend/
  ...
  verify.mjs        헤드리스 검증 하니스 — 단정 936건
  tools/
    pack_assets.py  CC0 애셋 → 아틀라스 파이프라인 (의존성 0)
  assets/
    sprites.png     39프레임 아틀라스 (CC0 유래, 49KB)
    manifest.json   프레임 표 + 라이선스/명도 메타
...
node frontend/verify.mjs          # 936 passed, 0 failed 를 기대
