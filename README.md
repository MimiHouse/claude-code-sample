...
**① 시체는 바닥에 눕고, 빠르게 사라집니다.** 붕괴는 발 위치를 **죽은 지점에서 그 아래 표면까지**
보간합니다. 처음에는 서 있던 발 높이에서 7px 가라앉혔는데, 그건 누운 몸을 **누워야 할 바닥에
파묻는** 것이고, 공중에서 죽은 몸에는 아무 효과도 없어서 죽은 높이에 그대로 떠 있었습니다.
이제 골목 바닥이든 엄폐 블록이든 2층 보행로든 **자기 아래에 있는 것 위에** 정착합니다.
구덩이 위에는 답이 없으니 계속 떨어져 시야에서 사라집니다 — 없는 바닥에 착지하는 척하지 않습니다.

![공중에서 사살](docs/kill-air.png)

`down` 프레임은 눈으로 고르지 않고 **종횡비로** 골랐습니다 — 바닥에 누운 몸은 넓고 낮습니다.
죽음 애니메이션은 각각 10프레임 전체이고 내용 종횡비가 어디서 착지하는지 정확히 알려줍니다
(SamuraiHeavy 0.90→0.58→**1.61**, SamuraiLight 0.75→0.66→**1.74**, Ninja Girl 0.59→**1.51** 정착).
첫 시도는 두 사무라이 모두 6번 프레임(0.79 / 1.12 — 겨우 기울어진 상태)을 써서 **시체가 서
있는 것처럼** 읽혔습니다. 걷기 위상과 같은 방식으로, 눈대중이 아니라 측정으로 뽑습니다.

소멸 창은 0.58s / 0.46s → **0.40s / 0.30s**이고 페이드는 66%가 아니라 55% 지점에서 시작합니다.
오래 남는 시체는 소품이고, 이 골목은 지나가기 위한 곳입니다.
...
| `shot-kill-air.png` | 공중에서 사살 — 떨어져 바닥에 정착 |
